<template>
    <div>
        <h1>评论组件</h1>
        <textarea v-model="com_value" maxlength="120" placeholder="请输入要BB的内容（最多吐槽120字）"></textarea>
        ...
        <div class="cmt-list">
            <div class="cmt-item" v-for="(item,index) in commentsList" :key="index">
                <div class="cmt-title">
                    第{{index+1}}楼 {{ item.username }} 发表时间 {{item.com_time | dateFormat}}
                </div>
